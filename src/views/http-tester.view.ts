...
import { HttpTesterGenerator } from '../generators/http-tester.generator';
...
export class HttpTesterPanel {
    ...
    private _socketState: SocketTesterState = { // Estado inicial del socket
    ...
    private _getHtmlForWebview(): string {
        const htmlPath = vscode.Uri.joinPath(this._extensionUri, 'media', 'http-tester.html');
    ...
    private _postSocketLog(message: string, type: 'info' | 'error' | 'event' = 'info') {
    ...
    private _updateSocketStatus(status: 'Connected' | 'Connecting' | 'Disconnected', className: string) {
        this._lastSocketStatus = { text: status, class: className };
        this._panel.webview.postMessage({ command: 'socketStatus', status: status, className: className });
    ...
    private _connectSocket(state: SocketTesterState) {
        ...
        this.socket.on("connect", () => {
            ...
            this.socket?.onAny((event, ...args) => {
                this._postSocketLog(`Event: ${event}\nPayload: ${JSON.stringify(args, null, 2)}`, 'event');
            });
        });
    ...
    private _emitEvent(data: { eventName: string, payload: string }) {
        if (!this.socket || !this.socket.connected) {
            ...
            this._panel.webview.postMessage({ command: 'error', message: 'Socket is not connected.' });
            return;
        }
        ...
        try {
            ...
            this._postSocketLog(`Emitted: ${data.eventName}\nPayload: ${data.payload}`, 'info');
        } catch (err: any) {
            this._postSocketLog(`Payload Error: Invalid JSON - ${err.message}`, 'error');
            this._panel.webview.postMessage({ command: 'error', message: `Invalid JSON Payload: ${err.message}` });
        }
    }
